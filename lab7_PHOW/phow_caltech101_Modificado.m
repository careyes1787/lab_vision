function phow_caltech101_Modificado()
...
conf.calDir_test = 'D:\Docs\Documents\UNIVERSIDAD\ULTIMO\Vision_en_Computador\LabPHOW\imagenet_small\test' ;
conf.calDir_train = 'D:\Docs\Documents\UNIVERSIDAD\ULTIMO\Vision_en_Computador\LabPHOW\imagenet_small\train' ;
conf.dataDir = 'data_New/' ;
...
%                                                           Setup data
% --------------------------------------------------------------------
% Se tiene en cuenta las imagenes de train y de test para cada una de las
% etapas correspondientes
%Train
classes_train = dir(conf.calDir_train) ;
classes_train = classes_train([classes_train.isdir]) ;
classes_train = {classes_train(3:conf.numClasses+2).name} ;

images_train = {} ;
imageClass_train = {} ;
for ci = 1:length(classes_train)
  ims = dir(fullfile(conf.calDir_train, classes_train{ci}, '*.JPEG'))' ;
  ims = vl_colsubset(ims, conf.numTrain+conf.numTest) ;
  ims = cellfun(@(x)fullfile(classes_train{ci},x),{ims.name},'UniformOutput',false) ;
...
for ci = 1:length(classes_train)
  ims = dir(fullfile(conf.calDir_test, classes_train{ci}, '*.JPEG'))' ;
  ims = vl_colsubset(ims, conf.numTrain+conf.numTest) ;
  ims = cellfun(@(x)fullfile(classes_train{ci},x),{ims.name},'UniformOutput',false) ;
...
selTrain = find(mod(0:length(images_train)-1, conf.numTrain+conf.numTest) < conf.numTrain) ;
selTest = setdiff(1:length(images_train), selTrain) ;
imageClass = cat(2, imageClass_train{:}) ;

model.classes = classes_train ;
...
if ~exist(conf.vocabPath) || conf.clobber
  ...
  parfor ii = 1:length(selTrainFeats)
    im = imread(fullfile(conf.calDir_train, images_train{selTrainFeats(ii)})) ;
...
if ~exist(conf.histPath) || conf.clobber
  ...
  % for ii = 1:length(images)
    ...
    im = imread(fullfile(conf.calDir_train, images_train{ii})) ;
    hists_train{ii} = getImageDescriptor(model, im);
  end 
  parfor ii = 1:length(images_test)
    fprintf('Processing %s (%.2f %%)\n', images_test{ii}, 100 * ii / length(images_test)) ;
    im = imread(fullfile(conf.calDir_test, images_test{ii})) ;
    hists_test{ii} = getImageDescriptor(model, im);
  end

  hists = cat(2, hists_train{:},hists_test{:}) ;
...
if ~exist(conf.modelPath) || conf.clobber
  switch conf.svm.solver
    case {'sgd', 'sdca'}
      ...
      w = [] ;
      parfor ci = 1:length(classes_train)
...
idx = sub2ind([length(classes_train), length(classes_train)], ...
              imageClass(selTest), imageEstClass(selTest)) ;
confus = zeros(length(classes_train)) ;
...
set(gca, 'ytick', 1:length(classes_train), 'yticklabel', classes_train) ;
subplot(1,2,2) ;
imagesc(confus) ;
